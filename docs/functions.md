...
|  ----  |  ----  |  ---- | ---- | ---- |
| 二次元图 | 在群中发送设定好的关键词即可 | 所有人 | 是 | `添加功能关键词#setu#[关键词]` <br> `删除功能关键词#setu` |
| 关键词搜索二次元图 | 在群中发送 `来点%keyword%[涩/色]图` 即可 | 所有人 | 是 | 功能开关跟随二次元图 |
| 二次元线稿图 | 在群中发送设定好的关键词即可 | 所有人 | 是 | `添加功能关键词#sketch#[关键词]` <br> `删除功能关键词#sketch` |
| 三次元图 | 在群中发送设定好的关键词即可 | 所有人 | 是 | `添加功能关键词#real#[关键词]` <br> `删除功能关键词#real` |
| 高质量三次元图 | 在群中发送设定好的关键词即可 | 所有人 | 是 | `添加功能关键词#realHighq#[关键词]` <br> `删除功能关键词#realHighq` |
| 壁纸 | 在群中发送设定好的关键词即可 | 所有人 | 是 | `添加功能关键词#bizhi#[关键词]` <br> `删除功能关键词#bizhi` |
| 图片存储功能 | 在群中发送 <br> <code>添加(setu/setu18/real/realHighq/wallpaper/sketch)图片[图片]+</code> 即可 | 2+ | 否 |  |
| 番剧查询 | 在群中发送 `番剧 %bangumiName%` 即可 | 所有人 | 否 | `%bangumiName%` 为番剧名 |
| 以图搜图 | 在群中发送 `搜图` 然后等待响应发送图片即可 | 所有人 | 是 | 只支持PIXIV完整图片搜索 <br> API: SauceNAO |
| 番剧以图搜图 | 在群中发送 `搜番` 然后等待响应发送图片即可 | 所有人 | 是 | API: trace.moe |
...
| 获取群年内词云 | 在群中发送 `本群年内总结` 即可 | 2+ | 否 | 自消息发出时向前一年内的所有被记录的消息 |
| 获取群月内词云 | 在群中发送 `本群月内总结` 即可 | 2+ | 否 | 自消息发出时向前一月内的所有被记录的消息 |
| 编译器 | 在群中发送 `super 语言名`(如py3) (换行) `code` 即可 | 所有人 | 是 | 支持的语言：R, vb, ts, kt, pas, lua, node.js, go, swift, rs, sh, pl, erl, scala, cs, rb, cpp, c, java, py3, py, php |
...
2. 生成各种表情包时，“qq id”之前需加空格，“@某人”与“图片”则不论空格加否均能响应；
